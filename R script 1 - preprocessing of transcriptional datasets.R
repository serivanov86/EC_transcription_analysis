...
tt<-fread("./Transcription data from GEO/GSE87620_non_normalized.txt",data.table=F)
rownames(tt)<-tt$ID_REF
tt<-tt[,-1]

# creation of EListRaw object
...
tt<-fread("./Transcription data from GEO/GSE25669_non_normalized.txt",data.table=F)
rownames(tt)<-tt$ID_REF
tt<-tt[,-1]
...
cels <- list.files("./Transcription data from GEO/", pattern="CEL")
tt <- ReadAffy(filenames = paste("./Transcription data from GEO/",cels,sep=""), cdfname='hgu133acdf')
